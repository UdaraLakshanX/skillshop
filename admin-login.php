                ...
                <div id="step1" class="space-y-6">
                    <form onsubmit="adminLogin(event);" class="space-y-6">
                        <div>
                            ...
                            <input type="email" id="email" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="admin@skilshop.com" required />
                        </div>
                        <button type="submit" id="loginBtn" class="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-black transition-all shadow-lg active:scale-[0.98]">Send Verification Code</button>
                    </form>
                </div>

                <!-- Step 2: Verification (Default hidden) -->
                <div id="step2" class="hidden space-y-6">
                    <form onsubmit="adminVerify(event);" class="space-y-6 text-center">
                        <div>
                            <label for="vcode" class="block text-slate-700 text-sm font-bold mb-4">Enter 6-Digit Code</label>

                            <input type="text" id="vcode" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-center text-3xl font-bold tracking-[0.5em]" placeholder="000000" maxlength="6" required />
                        </div>

                        <button type="submit" id="verifyBtn" class="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-black transition-all shadow-lg active:scale-[0.98]">Verify & Login</button>

                        <button type="button" onclick="goToStep1();" class="text-xs text-slate-400 hover:text-slate-600 font-bold uppercase tracking-widest">Change Email</button>
                    </form>
                </div>
