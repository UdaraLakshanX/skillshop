...
if (!isset($_SESSION["logged_in"]) || !$_SESSION["logged_in"]) require_once "process/auth-check.php";
...
// Stats
$rs = Database::search("SELECT AVG(`rating`) AS `r`, COUNT(*) AS `c` FROM `feedback` WHERE `product_id`=?",
"i",[$productId])->fetch_assoc();
$sc = Database::search("SELECT COUNT(DISTINCT `user_id`) AS `c` FROM `order` WHERE `product_id`=?",
"i",[$productId])->fetch_assoc();
...
$revQ = Database::search(
    "SELECT f.*, u.`fname`, u.`lname` FROM `feedback` f LEFT JOIN `user` u ON f.`user_id`=u.`id` WHERE f.`product_id`=?
...
$ss = Database::search(
    ...
    FROM `product` p LEFT JOIN `order` o ON p.`id`=o.`product_id` LEFT JOIN `feedback` f ON p.`id`=f.`product_id` 
...
$inWatchlist = false;
if($loggedIn && $userId){
    $wq = Database::search("SELECT `id` FROM `watchlist` WHERE `product_id`=? AND `user_id`=? LIMIT 1","ii",[$productId,$userId]);
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-blue-900" style="min-height:280px;">
        ...
        <div class="relative max-w-6xl mx-auto px-4 pt-10 pb-16">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                <?php endif; ?>
                <?php if($bought): ?>
                    <span class="inline-flex items-center text-xs font-bold px-3 py-1 rounded-full bg-green-500/25 text-green-300">✔️ Enrolled</span>
                <?php endif; ?>
            </div> 

            <h1 class="text-3xl md:text-4xl font-extrabold text-white leading-tight mb-4 max-w-2xl"><?= $p["title"]?></h1>
            ...
            <div class="flex-1 min-w-0 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <?php foreach(["Full course access","Lifetime access","Direct support","Resources","Certificate","Expert content"] as $f): ?>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-3 p-4 rounded-2xl bg-slate-50">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <?php if($rv["message"]): ?>
                                <p class="text-sm text-gray-600 mt-1"><?= nl2br($rv["message"]) ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endforeach; ?>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm">
                    ...
                    <div class="grid sm:grid-cols-3 gap-4">
                        ...
                        <a href="product-view.php?id=<?= $r["id"] ?>" class="group block rounded-xl overflow-hidden border border-slate-100 hover:border-blue-200
                        hover:shadow-md transition-all">
                            ...
                            <div class="p-3">
                                ...
                                <p class="text-sm font-bold text-blue-600 mt-1">Rs. <?= number_format($r["price"],2) ?></p>
                            </div>
                        </a>
                        <?php endforeach; ?>
                    </div>
                ...
                <div class="sticky top-24 space-y-4">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xl">
                        ...
                        <div class="p-5">
                            ...
                            <!-- Enroll / Status -->
                            <?php if($bought): ?>
                                <div class="flex items-center justify-center gap-2 py-3 mb-3 rounded-xl text-sm font-bold
                                text-green-700 bg-green-50 border-2 border-green-300">✓ Already Enrolled</div>
                                <a href="buyer-dashboard.php?tab=learning" class="block w-full py-3.5 text-center 
                                bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700
                                text-white font-bold rounded-xl transition-all active:scale-95 mb-3">
                                Go to My Learnings</a>
                            <?php elseif($loggedIn && $userRole == "buyer"): ?>
                                <a href="process/checkout.php?id=<?= $p["id"]; ?>" class="block w-full py-3.5 text-center bg-gradient-to-r from-blue-600
                                to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl
                                transition-all active:scale-95 shadow-md mb-3">🎓 Enroll Now</a>

                                <!-- Cart / Watchlist -->
                                <div class="flex gap-2 mb-4">
                                    <button id="cart-btn" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border-2
                                    font-semibold text-sm transition-all border-slate-200 text-slate-600 hover:border-blue-400 
                                    hover:text-blue-600" data-product-id="<?= $productId; ?>" data-in="<?= $inCart ? 1 : 0; ?>">🛒 <span id="cart-text"><?= $inCart ? "In Cart" : "Add to Cart"; ?></span></button>

                                    <button id="wl-btn" class="flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border-2 font-semibold text-sm transition-all border-slate-200 text-slate-600 hover:border-rose-400 hover:text-rose-600" data-product-id="<?= $productId; ?>" data-in="<?= $inWatchlist ? 1 : 0; ?>">
                                        <?= $inWatchlist ? "❤︎" : "♡"; ?>
                            ...
                            <?php elseif(!$loggedIn): ?>
                                ...
                                <a href="index.php" class="block w-full py-3.5 text-center bg-gradient-to-r from-blue-600
                                to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl
                                transition-all active:scale-95 shadow-md mb-3">Sign In to Enroll</a>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-xl">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            font-bold text-lg bg-gradient-to-br from-blue-600 to-indigo-600"><?= strtoupper($p["fname"][0]) ?? "S" ?></div>
                            <div>
                        ...
                        <div class="grid grid-cols-3 gap-2 text-center mb-4">
                            <div class="rounded-xl py-2 bg-blue-50">
                                <p class="font-extrabold text-sm text-blue-600"><?= intval($ss["tc"]) ?></p>
                            ...
                            <div class="rounded-xl py-2 bg-yellow-50">
                                <p class="font-extrabold text-sm text-yellow-600"><?= $ss["tr"] > 0 ? round($ss["tr"],1) : "-"; ?></p>
