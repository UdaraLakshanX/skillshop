...
<div class="min-h-screen bg-gray-50">
    ...
    <?php elseif($tab == "cart"): ?>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <?php if(empty($cartItems)): ?>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-16 text-center shadow-sm">
                    ...
                    <a href="search-products.php" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-all shadow-lg active:scale-95">Browse Skills</a>
                </div>
                ...
                <div class="flex flex-col lg:flex-row gap-8">
                    <div class="flex-1 space-y-4">
                        ...
                        <?php foreach($cartItems as $item): 
                            ...
                            ?>
                            <!-- Cart Item -->
                            <div onclick='openProductModal(<?= json_encode($productData) ?>);' id="cart-item-<?= $item["cart_item_id"]; ?>" class="cursor-pointer bg-white rounded-2xl border border-slate-100  p-4 flex gap-4 shadow-sm hover:shadow-md transition-shadow group">
                                    ...
                                    <div class="flex justify-between items-start">
                                        ...
                                        <button onclick="removeItem(<?= $item['cart_item_id'] ?>, event);" class="text-slate-300 hover:text-rose-500 p-1 transition-colors" title="Remove">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"     stroke="currentColor" class="w-6 h-6">
                                                ...
                                                d="M3 6h18M9 6v12m6-12v12M5 6l1 14a2 2 0 002 2h8a2 2 0 002-2l1-14M10 6V4a1 1 0 011-1h2a1 1 0 011 1v2" />
                                            </svg>
                                        </button>
                                    </div>
                    ...
                    <aside class="lg:w-96 flex-shrink-0">
                        <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-xl sticky top-24">
                                ...
                                <div class="flex justify-between text-slate-600">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <div class="group relative">
                                            ...
                                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-900 text-white text-[10px] rounded shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                            ...
                            <div class="pt-6">
                                ...
                                <button onclick="checkout();" class="block w-full py-4 text-center bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all active:scale-95 mb-4">
                                    Proceed to Checkout
                                </button>
        ...
        <div class="fixed inset-0 z-[100] hidden" id="productModal">
            <!-- Overlay -->
            <div id="modalOverlay" class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm opacity-0 transition-opacity duration-300" onclick="closeProductModal();"></div>

            <!-- Modal Content -->
            <div id="modalContent" class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-11/12 max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row transform scale-95 opacity-0 transition-all duration-300">
                ...
                <div class="md:w-1/2 relative bg-slate-100 aspect-video md:aspect-auto">
                    ...
                    <span id="modalLevel" class="absolute top-4 left-4 text-xs font-bold px-3 py-1 rounded-md bg-blue-600 text-white uppercase tracking-wider shadow-md">
                        Beginner
                    </span>
                </div>
                ...
                <div class="p-8 md:w-1/2 flex flex-col relative">
                    ...
                    <!-- Close Button -->
                    <button onclick="closeProductModal();" class="absolute top-5  right-5 text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-full w-10 h-10 flex items-center justify-center transition-colors">
                        X                
                    </button>
        ...
        <?php if(!empty($cartItems)): ?>
            ...
            <script>
                ...
                function openProductModal(data){
                    ...
                    document.getElementById("modalPrice").innerText = "Rs. " + parseFloat(data.price).toLocaleString(undefined, {minimumFractionDigits: 2});
    ...
    <main class="max-w-7xl mx-auto px-4 lg:px-8 py-10">
                        ...
                        <div class="divide-y divide-gray-100">
                            <?php
                                $itemsQ =  Database::search(
                                    "SELECT ii.*, p.`title`,  p.`image_url`, u.`fname` AS `seller_first`, u.`lname` AS `seller_last`, (SELECT `id` FROM `feedback` WHERE `user_id`=? AND `product_id`=ii.`product_id` LIMIT 1) AS `feedback_id`
                                    FROM `invoice_item` ii 
                                    JOIN `product` p ON ii.`product_id`=p.`id`
                                    JOIN `user` u ON ii.`seller_id`=u.`id`
                                    WHERE ii.`invoice_id`=?", 
                                    "ii",[$userId, $invoice["id"]]
                                );
                            ...
                            ?>
                                <a href="product-view.php?id=<?= $item["product_id"] ?>" class="p-6 flex flex-col md:flex-row gap-6 items-start md:items-center hover:bg-blue-200 transition-colors">
                                    ...
                                    <div class="flex-shrink-0 w-full md:w-auto">
                                        ...
                                        <?php else: ?>
                                            <button onclick="openFeedbackModal(<?= $item['product_id'] ?>, '<?= addslashes($item['title']) ?>', event)" class="w-full md:w-auto px-6 py-2.5 bg-gray-900 text-white font-bold rounded-xl shadow-sm hover:bg-black transition-all text-sm">
                                                Give Feedback
                                            </button>
                                        <?php endif; ?>
                                    </div>
        ...
        <?php else: ?>
            <div class="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-100">
                <div class="w-20 h-20 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6 text-3xl">🛍️</div>
                ...
                <div class="p-8">
                    ...
                    <form id="feedback-form" onsubmit="submitFeedback(event);">
                        ...
                        <div class="flex gap-4">
                            ...
                            <button type="submit" class="flex-1 px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">Submit</button>
                        </div>
    ...
    <script>
        ...
        async function submitFeedback(e){
            ...
            try {
                const res = await fetch("process/saveFeedback.php",{ method:"POST", body: fd});          
                const data = await res.json();
                if(data.success){
